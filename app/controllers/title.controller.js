	...
	Title.findById(req.params.title_id, (err, data) => {
		if (err) {
			if (err.kind === "not_found") {
				...
				res.status(500).send({
					message:
						"Error retrieving title with title_id " + req.params.title_id,
				});
			}
		} else res.send(data);
	});
		...
		(err, data) => {
			if (err) {
				if (err.kind === "not_found") {
					...
					res.status(500).send({
						message:
							"Error updating title with title_id " + req.params.title_id,
					});
				}
			} else res.send(data);
		}
	...
	Title.remove(req.params.title_id, (err, data) => {
		if (err) {
			if (err.kind === "not_found") {
				...
				res.status(500).send({
					message:
						"Could not delete title with title_id " + req.params.title_id,
				});
			}
