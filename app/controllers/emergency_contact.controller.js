	...
	emergencyContact.create((err, data) => {
		if (err) {
			res.status(500).send({
				message:
					err.message ||
					"Some error occurred while creating the emergency contact",
			});
		} else res.send(data);
	});
};
	...
	EmergencyContact.findById(req.params.emergency_contact_id, (err, data) => {
		if (err) {
			if (err.kind === "not_found") {
				...
				res.status(500).send({
					message:
						"Error retrieving emergency contact with emergency_contact_id " +
...
// retrieve all emergency contacts from the database for given emp_id
export const findByEmpID = (req, res) => {
	const emp_id = req.params.emp_id;
	EmergencyContact.getByEmp(emp_id, (err, data) => {
		if (err)
			res.status(500).send({
				message:
					err.message ||
		...
		(err, data) => {
			if (err) {
				if (err.kind === "not_found") {
					...
					res.status(500).send({
						message:
							"Error updating emergency contact with emergency_contact_id " +
							req.params.emergency_contact_id,
					});
				}
			} else res.send(data);
	...
	EmergencyContact.remove(req.params.emergency_contact_id, (err, data) => {
		if (err) {
			if (err.kind === "not_found") {
				...
				res.status(500).send({
					message:
						"Could not delete emergency contact with emergency_contact_id " +
						req.params.emergency_contact_id,
				});
			}
